...
class File(BaseModel):
    ...
    created_at: Optional[datetime] = Field(default=None, description="文件创建时间", alias="createdAt")
    updated_at: Optional[datetime] = Field(default=None, description="文件更新时间（上传文件时更新），默认为创建时间", alias="updatedAt")
    modified_at: Optional[datetime] = Field(default=None, description="文件修改时间（词条修改时更新），默认为创建时间；可利用此字段判断文件中词条是否有更新", alias="modifiedAt")
